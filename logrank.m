...
if ~isempty(table12)
    S3=plot(xcg1,ycg1,'k+','LineWidth',1); %Censored data for treatment 1 (if there are)
else
    S3=[];
end
...
if ~isempty(table22)
    S3=plot(xcg2,ycg2,'k+','LineWidth',1); %Censored data for treatment 2 (if there are)
end
hold off
...
xlabel('Time')
if isempty(S3)
    h=legend([S1 S2],'Treatment 1','Treatment 2')
else
    h=legend([S1 S2 S3],'High risk','Low risk','Censored')
end
